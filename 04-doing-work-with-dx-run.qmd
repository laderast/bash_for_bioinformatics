...
In your shell, make sure you're in the `bash_bioinfo_scripts/04-doing-work-with-dx-run/` folder:

```
cd 04-doing-work-with-dx-run/
```
...
When you run the above code, either by pasting it into your shell, or using `bash run_sambamba.sh` you'll see the following response. 
...
```{bash}
#| eval: false
#| filename: 04-doing-work-with-dx-run/run-on-worker.sh
...
```{bash}
#| eval: false
#| filename: 04-doing-work-with-dx-run/dx_run_script.sh
...
The magic here is that we're using `bash` in our `cmd` input to run the script `run_on_worker.sh`. Notice this script is already in our project in the `worker_scripts/` folder, so we can submit it as a file input:
...
```{bash}
#| eval: false
#| filename 04-doing-work-with-dx-run/run_plink.sh
...
```{bash}
#| eval: false
#| filename: 04-doing-work-with-dx-run/run_destination.sh
dx run app-swiss-army-knife \
  -iin="data/chr1.bed" \
  -iin="data/chr1.bim" \
  -iin="data/chr1.fam" \
...
```{bash}
#| eval: false
#| filename: 04-doing-work-with-dx-run/dx_run_instance.sh
dx run app-swiss-army-knife \
  -iin="data/chr1.bed" \
  -iin="data/chr1.bim" \
  -iin="data/chr1.fam" \
...
```{bash}
#| eval: false
#| filename: 04-doing-work-with-dx-run/run_job_tag_name.sh
dx run app-swiss-army-knife \
  -iin="data/chr1.bed" \
  -iin="data/chr1.bim" \
  -iin="data/chr1.fam" \
...
```{bash}
#| eval: false
#| filename: 04-doing-work-with-dx-run/run_job_dxfuse.sh

dx run app-swiss-army-knife \
  -icmd="plink --bfile /mnt/project/data/chr1 --geno 0.01 --make-bed --out chr1_qc"
```

In our `-icmd` input, we are referring to the file location of the triplet by pre-pending a `/mnt/project/` to the project path of the triplet: `data/chr1`. 
