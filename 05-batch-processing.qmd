...
```{bash}
#| eval: false
#| filename: 05-batch-processing/dx_find_path.sh
source ~/.bashrc
...
```{bash}
#| eval: false
#| filename: 05-batch-processing/dx_find_data_field.sh
source ~/.bashrc
...
```{bash}
#| eval: false
#| filename: 05-batch-processing/dx_find_data_class.sh
source ~/.bashrc
...
```{bash}
#| eval: false
#| filename: 05-batch-processing/dx_find_xargs.sh
dx find data --name "*.bam" --brief | \
...
## Submitting Mutltiple Files to Swiss Army Knife {#sec-mult-worker}

We talked about another method to batch process files on a worker (@fig-batch2). We can submit an array of files to a worker, and then process them one at a time on a worker.
