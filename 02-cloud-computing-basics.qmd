...
Let's run through the order of operations of running a job on the platform in @fig-order-operations. Let's focus on running an aligner (BWA-MEM) on a FASTQ file. Our output will be a .BAM (aligned reads) file.

![Order of operations of a job](images/order_operations.png){#fig-order-operations}
Let's go over the order of operations needed to execute our job on the DNAnexus platform.
